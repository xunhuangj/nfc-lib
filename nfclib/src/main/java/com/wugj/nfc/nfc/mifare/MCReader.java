...
public class MCReader {
    ...
    //存放所有能开启每一个扇区的秘钥，秘钥都是一对 keyA and keyB
    ...
    //save all keys  every key is a six byte array
    private ArrayList<byte[]> mKeysWithOrder;

    public ArrayList<byte[]> getmKeysWithOrder() {
        return mKeysWithOrder;
    }

    private Tag tag = null;
    private MCConfig mcConfig = null;


    public int getmLastSector() {
        return mLastSector;
    }

    public MCReader(Tag tag, MCConfig mcConfig) {
        this.tag = tag;
        if (mcConfig != null)
            this.mcConfig = mcConfig;
        else
            throw new NullPointerException("new MCReader: mcConfig is null");
     ...
     * not MIFARE Classic.
     */
    public static MCReader get(Tag tag, MCConfig mcConfig) {
        MCReader mcr = null;
        if (tag != null) {
            try {
                mcr = new MCReader(tag, mcConfig);
     ...
     * (use {@link #setmKeysWithOrder(ArrayList)}).
     ...
     * @see #buildNextKeyMapPart()
     * @see #setmKeysWithOrder(ArrayList)
     */
    public SparseArray<String[]> readAsMuchAsPossible(boolean isReadKeyMapPart) {
        if (isReadKeyMapPart) {
    ...
    //
    public int getBlockNums(int sectorIndex) {
        ...
        if (auth) {
            ...
            for (int i = firstBlock; i < lastBlock; i++) {
                try {
                    ...
                    blocks.add(Utils.byte2HexString(blockBytes));
                } catch (TagLostException e) {
                    throw e;
                } catch (IOException e) {
            ...
            if (noData) {
                ...
                if (!useAsKeyB) {
                    if (isKeyBReadable(Utils.hexStringToByteArray(
                            ret[last].substring(12, 20)))) {
                        ret[last] = Utils.byte2HexString(key)
     ...
     * @param startBlock
     * @param endBlock
     * @param keybytes    Key for authentication.
     *                    for authentication.
     * @return Array of blocks (index 0-3 or 0-15). If a block or a key is
     * marked with {@link #NO_DATA} or {@link #NO_KEY}
     * it means that this data could not be read or found. On authentication error
     ...
     * @see #mergeSectorData(String[], String[])
     */
    public String[] readSector(int sectorIndex, int startBlock, int endBlock, byte[][] keybytes
    ) throws TagLostException {

        if (keybytes == null) throw new NullPointerException("read sector = " + sectorIndex +
        ...
        if (auth) {
            // Read all blocks.
            ArrayList<String> blocks = new ArrayList<>();
            //endBlock index = 0 start.
            int firstBlock = mMFC.sectorToBlock(sectorIndex);
            startBlock += firstBlock;
            int lastBlock = firstBlock + endBlock + 1;
    ...
    public int buildNextKeyMapPart() {
        ...
        if (mKeysWithOrder != null && mLastSector != -1) {
            ...
            MLog.i(LOG_TAG, new StringBuilder("mKeyMapStatus = ").append(mKeyMapStatus)
                    .append(" mFirstSector = ").append(mFirstSector)
                    .append(" mLastSector = ").append(mLastSector)
                    .toString());
            ...
            for (int i = 0; i < mKeysWithOrder.size(); i++) {
                ...
                for (int j = 0; j < mcConfig.getRetryAuthCount() + 1; ) {
                    try {
                        if (!foundKeys[0]) {
                        ...
                        Log.d(LOG_TAG,
                                "Error while building next key map part" + e.getMessage());
                        // Is auto reconnect enabled?
                        if (mcConfig.isAutoConnected()) {
                    ...
                    // Retry?
                    if ((foundKeys[0] && foundKeys[1]) || !mcConfig.isRetryAuth()) {
            ...
            MLog.i(LOG_TAG, new StringBuilder("error = ").append(error)
                    .append(" foundKey[0] = ").append(foundKeys[0])
                    .append(" foundKey[1] = ").append(foundKeys[1])
                    .toString());
...
//                mKeyMap.put(mKeyMapStatus, null);
            }
            mKeyMapStatus++;
        } else {
            error = true;
    ...
    public byte[][] createAuthKeys(int sectorIndex) {
        // Clear status and key map before new walk through sectors.
        ...
        if (mKeysWithOrder == null)
            throw new IllegalArgumentException("mKeysWithOrder is null,you must set mKeysWithOrder");
        ...
        for (int i = 0; i < mKeysWithOrder.size(); i++) {
            ...
            for (int j = 0; j < mcConfig.getRetryAuthCount() + 1; ) {
                try {
                    ...
                    throw new IllegalStateException("Error while building next key map part" + e.getMessage());
                }
                // Retry?
                if ((foundKeys[0] && foundKeys[1]) || !mcConfig.isRetryAuth()) {
                    // Both keys found or no retry wanted. Stop retrying.
    ...
    public HashMap<Integer, HashMap<Integer, Integer>> isWritableOnPositions(
        ...
        for (int i = 0; i < keyMap.size(); i++) {
            ...
            if (pos.containsKey(sector)) {
                ...
                byte[][] acMatrix = MCCommon.acBytesToACMatrix(ac);
                if (acMatrix == null) {
                    ret.put(sector, null);
                    continue;
                }
                boolean isKeyBReadable = MCCommon.isKeyBReadable(
                ...
                for (int block : pos.get(sector)) {
                    if ((block == 3 && sector <= 31)
                        ...
                        int acValue = MCCommon.getOperationInfoForBlock(
                                acMatrix[0][3],
                                acMatrix[1][3],
                                acMatrix[2][3],
                                MCCommon.Operations.WriteAC,
                                true, isKeyBReadable);
                        // Is key A writable? (If so, key B will be writable
                        // with the same key.)
                        int keyABValue = MCCommon.getOperationInfoForBlock(
                                acMatrix[0][3],
                                acMatrix[1][3],
                                acMatrix[2][3],
                                MCCommon.Operations.WriteKeyA,
                                true, isKeyBReadable);
    ...
    /**
     * before call read we need setKeys
     *
     * @param keysWithOrder
     */
    public void setmKeysWithOrder(ArrayList<byte[]> keysWithOrder) {

        mKeysWithOrder = keysWithOrder;
    }
    ...
    private boolean authenticate(int sectorIndex, byte[] key,
        ...
        boolean retryAuth = mcConfig.isRetryAuth();
        int retryCount = mcConfig.getRetryAuthCount();
        boolean ret = false;
     ...
     * the given key file ({@link #setmKeysWithOrder(ArrayList)}}). If you want a
    ...
    public void close()  {
        try {
            ...
            e.printStackTrace();
            throw new IllegalStateException("close tag exception");
        }
    }
}
